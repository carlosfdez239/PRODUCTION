...
from ls_message_parsing.utils.amcodes_utils import OUTPUT_AMTYPES
...
# import numpy
import lib.generic_modbus_cfgs as gmcfgs
...
class ParsingPriority:
    OLD_PARSING = 1
    PARSING_LIBRARY_BEFORE_CIR = 2
    PARSING_LIBRARY_WITH_CIR = 3
...
def msg2string(
    msg,
    parsing_priority=[
        ParsingPriority.OLD_PARSING,
        ParsingPriority.PARSING_LIBRARY_WITH_CIR,
        ParsingPriority.PARSING_LIBRARY_BEFORE_CIR,
    ],
):
    for priority in parsing_priority:
        out_str = ""
        try:
            if priority == ParsingPriority.OLD_PARSING:
                logger.debug("**Trying to parse with old parsing")
                out_str = _msg2string_old(msg)
                if "; Unknown" in out_str:
                    raise Exception("Unknown amtype in old parsing")
                return out_str
            elif priority == ParsingPriority.PARSING_LIBRARY_WITH_CIR:
                logger.debug("**Trying to parse with parsing library with CIR format")
                json_msg = _msg2json_cir(msg)
                return _json2string_cir(json_msg)
            elif priority == ParsingPriority.PARSING_LIBRARY_BEFORE_CIR:
                logger.debug("**Trying to parse with parsing library before CIR format")
                ...
                json_msg = _msg2json_before_cir(msg)
                msg_type = json_msg.pop("type")
                if msg_type:
                    header += f"{msg_type}; "
                return header + _json2string_before_cir(json_msg, msg_type=msg_type)
        except Exception as e:
            logger.debug(f"**Error: {e}")
            logger.debug("**Falling back to next parsing method")
...
def _msg2json_cir(binary_msg):
    out = _decode_msg(binary_msg, format="CIR")
    msg = out.get_decoded_message_dict()
    msg["msg_name"] = out.msg_obj.__name__
    return msg


def _msg2json_before_cir(binary_msg):
...
def msg2json(
    ...
    for priority in parsing_priority:
        try:
            if priority == ParsingPriority.PARSING_LIBRARY_WITH_CIR:
                logger.debug("**Trying to parse with parsing library with CIR format")
                msg = _msg2json_cir(binary_msg)
                return msg
            elif priority == ParsingPriority.PARSING_LIBRARY_BEFORE_CIR:
                logger.debug("**Trying to parse with parsing library before CIR format")
                msg = _msg2json_before_cir(binary_msg)
                return msg
            elif priority == ParsingPriority.OLD_PARSING:
                logger.debug("**Trying to parse with old parsing")
                msg = _msg2json_old(binary_msg)
                if "unknown" in msg.get("type", ""):
                    raise Exception("Unknown amtype in old parsing")
                return msg
        except Exception as e:
            logger.debug(f"**Error: {e}")
...
def _get_sane_format_before_cir(msg_type, json_data):
...
def _json2string_before_cir(json_msg, msg_type=None):
    beautified_string = _get_sane_format_before_cir(msg_type, json_msg)
...
def cir_simplify_value(item):
    """
    Recursively simplifies a data structure by extracting the 'value'
    from dictionaries, and processing lists and other dicts.
    """
    if isinstance(item, dict) and "value" in item:
        return cir_simplify_value(item["value"])
    elif isinstance(item, dict):
        return {k: cir_simplify_value(v) for k, v in item.items()}
    elif isinstance(item, list):
        return [cir_simplify_value(elem) for elem in item]
    return item


def _json2string_cir(data: dict) -> str:
    """
    Converts sensor data into a simplified string.
    Handles both raw (nested) and pre-simplified data structures.
    """
    # --- 1. Manually process the fixed header robustly ---

    # Safely get header and timestamp, checking if they are dicts or simple values
    header = data.get("ls_header", {})

    # Safely get product_code
    pr_code_val = header.get("product_code")
    pr_code = pr_code_val.get("value") if isinstance(pr_code_val, dict) else pr_code_val

    # Safely get node_id
    node_id_val = header.get("node_id")
    node_id = node_id_val.get("value") if isinstance(node_id_val, dict) else node_id_val

    # Safely get sequence_number
    seq_num_val = header.get("sequence_number")
    seq_num = seq_num_val.get("value") if isinstance(seq_num_val, dict) else seq_num_val

    amtype_val = header.get("am_type")
    amtype = amtype_val.get("value") if isinstance(amtype_val, dict) else amtype_val

    # Safely get readTimestamp or timestamp
    timestamp_val = (
        data.get("readTimestamp")
        if data.get("readTimestamp") is not None
        else data.get("timestamp")
    )
    timestamp = timestamp_val.get("value") if isinstance(timestamp_val, dict) else timestamp_val

    msg_name = "Unknown"
    try:
        # msg_name = OUTPUT_AMTYPES[int(amtype)]["msg_type"]
        msg_name = data.get("msg_name")

    except Exception:
        logger.error("Unknown amtype or error retrieving msg_type")
        logger.error(f"Amtype: {amtype}")
        if amtype in OUTPUT_AMTYPES:
            logger.error(f"Dict: {OUTPUT_AMTYPES[amtype]}")
    ...
    output_parts = [
        ...
        f"{msg_name}",
        f"am_type: {amtype}",
        f"readTimestamp: {timestamp}",
    ]

    # --- 2. Generically process all other fields ---
    keys_to_skip = {"ls_header", "readTimestamp", "timestamp", "padding", "msg_name"}

    for key, value in data.items():
        if key in keys_to_skip:
            continue

        simplified = cir_simplify_value(value)

        if isinstance(simplified, (list, dict)):
            val_str = _json.dumps(simplified)
        else:
            val_str = str(simplified)
